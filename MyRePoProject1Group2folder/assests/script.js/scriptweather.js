...
document.addEventListener('DOMContentLoaded', () => {
    const apiKey = '6bdabfafea6c9fb1c11b7b85ca98c4ca'; // Store securely in production
    const searchButton = document.querySelector('.button.is-primary'); // Button element
    const weatherResultEl = document.getElementById('weatherdataicon');
    const weatherCityEl = document.querySelector('.city'); // City element

    searchButton.addEventListener('click', () => {
        const inputCity = document.getElementById('search-result').value; // Get city name
        const inputCountry = document.getElementById('country-input').value; // Get country name (if you add this input)

        if (inputCity && inputCountry) {
            getWeather(inputCity, inputCountry);
        } else {
            console.log('Please fill in both city and country fields.');
        }
    });
    ...
    function getWeather(city, country) {
        ...
        fetch(requestUrlGeocode)
            ...
            .then(response => response.json())
            .then(weatherData => {
                weatherResultEl.innerHTML = ''; // Clear existing content
                weatherData.list.forEach(day => {
                    if (day.dt_txt.includes('12:00:00')) { // Only display midday forecast
                        const weatherCard = document.createElement('div');
                        weatherCard.className = 'column is-one-fifth';
                        ...
                        `;
                        weatherResultEl.appendChild(weatherCard);
                    }
                });
            })
            .catch(error => console.error('Error fetching weather data:', error));
